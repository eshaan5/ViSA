...
//document.getElementsById("merge").addEventListener("click", function(){
	//MergeSort(0, size-1);});
...
async function BubbleSort(delay = 300){
	...
	async function QuickSort(l, r, delay=100){
		...
		async function Merge(l, m, r, delay=400){
			var bars = document.querySelectorAll(".bar");
			var n1 = m - l + 1;
			var n2 = r - m;
			...
			for(var j=0;j<n2;j++){
				R[j] = parseInt(bars[m+1+j].childNodes[0].innerHTML);
			}

			var i = 0, j=0, k=l;

			while(i<n1 && j<n2){
				bars[l+k].style.backgroundColor = "orange";
				bars[m+1+k].style.backgroundColor = "red";
				...
				bars[l+k].style.backgroundColor = " rgb(24, 190, 255)";
				bars[m+1+k].style.backgroundColor = " rgb(24, 190, 255)";
			}
			while(i<n1){
		...
		async function MergeSort(l, r, delay=400){
			var bars = document.querySelectorAll(".bar");
			if(l<r){
...
document.getElementsById("new").disabled = true;
